...
# Backend Functions copied verbatim from original script
...
def get_perpendicular_dim_base(start, end, perp_dir, offset=None):
    if offset is None:
        offset = PERP_DIM_OFFSET
    mid = (
        (start[0] + end[0]) / 2,
        (start[1] + end[1]) / 2
    )
...
def draw_perpendicular_for_behind_hit(beam_p1, beam_p2, beams, beam_idx, msp,
    ...
    perp_length = RATIO * front_dist if front_dist else RATIO * behind_dist
    ...
    add_beam_spacing_text(
        ...
        text_template=SPACING_TEXT_TEMPLATE
    )
...
def generate_hidden_perpendiculars_like_find_hidden(beam_p1, beam_p2, beams,
    ...
    for behind_hit in nearest_hits:
        ...
        perp_length = RATIO * front_dist if front_dist else RATIO * behind_hit["dist"]
...
def find_hidden_and_generate(p1, p2, beams, beam_idx, msp,
    ...
    perp_length = RATIO * front_dist if front_dist else RATIO * nearest_dist
